...
Signal_USB::Signal_USB(float mod_idx, float f_max, float var1, float var2, float thresh, int seed,
    ...
    d_norm(norm)
{
  set_seed(seed);
  generate_taps();
  d_gm = Gaussian_Mixture(sqrt(d_var1),sqrt(d_var2),d_fmax,d_thresh,seed);
  d_burn = 20;
  d_agc.set_rate(5e-4);
  if(d_enable){
    d_running = true;
...
Signal_USB::generate_signal(complexf* output, size_t sample_count)
{
  ...
  for(size_t idx = sample_count/2+1; idx <= sample_count; idx++){
    d_fft_in[idx%sample_count][0] = 0.;
    d_fft_in[idx%sample_count][1] = 0.;
  }
...
void
Signal_USB::filter( float* n, float* m, size_t length )
{
