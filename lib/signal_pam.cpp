    ...
    d_symbol_count(0)
{
  ...
  if(d_enable){
    ...
    load_firs();
    //printf("PAM::Loaded FIR filters.\n");
    d_running = true;
    auto_fill_symbols();
    auto_fill_signal();
  }
  //printf("PAM::Fully made\n");
}

Signal_PAM::~Signal_PAM()
{
  if(d_enable){
    d_running = false;/*
...
void
Signal_PAM::generate_symbols(complexf* output, size_t symbol_count)
{
  if(d_enable){
    size_t filled(0);
...
void
Signal_PAM::generate_signal(complexf* output, size_t sample_count)
{
  size_t oo = 0;
...
void
Signal_PAM::create_symbol_list(int k)
{
...
void
Signal_PAM::filter( size_t nout, complexf* out )
{
...
void 
Signal_PAM::auto_fill_symbols()
{
  d_Sy = new signal_threaded_buffer<complexf>(d_buffer_size,d_notify_size);
  d_TGroup.create_thread( boost::bind(&Signal_PAM::auto_gen_SYMS, this) );
}

void 
Signal_PAM::auto_fill_signal()
{}


void
Signal_PAM::load_firs()
{
  //printf("PAM::Attempting to load filters.\n");
  size_t intp = d_sps;
  size_t ts = d_pulse_shape.size() / intp;
  if((d_pulse_shape.size() % intp)){
    ts++;
  }
  //printf("PAM:: Intp(%lu), ts(%lu), sps(%d), d_overlap(%lu).\n",intp,ts,d_sps,d_overlap);

  //std::vector< std::vector<float> > xtaps(intp);
  d_taps = std::vector< std::vector<float> >(intp);

  //printf("PAM:: taps_size(%lu).\n",d_taps.size());
  ...
  //printf("PAM:: filters made.\n");
  for(size_t idx = 0; idx < intp; idx++){
    d_firs[idx]->set_taps(d_taps[idx]);
  }
  //printf("PAM:: taps loaded.\n");
  d_hist = ts-1;
}


void
Signal_PAM::auto_gen_SYMS()
{
...
void
Signal_PAM::throw_runtime(std::string err, size_t sc, size_t os, size_t si)
{
  //printf("Error occured while trying to make %lu samples, on sample %lu, symbol_idx %lu\n",sc,os, si);
  //printf("Error occured (PAM): A= %lu, B= %lu, C= %lu\n",sc,os, si);
  d_running = false;
  d_TGroup.join_all();
  delete d_Sy;
  for(size_t idx = 0; idx < d_sps; idx++){
